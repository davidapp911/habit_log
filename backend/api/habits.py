...
@router.get("/", response_model=list[HabitResponse], status_code=200)
def list_habits(
...
@router.get("/{habit_id}", response_model=HabitResponse, status_code=200)
def get(
    habit_id: int,
    db: Session = Depends(database_session),
    user: User = Depends(current_user),
...
@router.put("/{habit_id}", response_model=HabitResponse, status_code=200)
def update(
    habit_id: int,
    body: HabitUpdate,
    db: Session = Depends(database_session),
...
@router.get(
    "/{habit_id}/logs", response_model=list[HabitCompletionResponse], status_code=200
)
def get_completions(
    habit_id: int,
    db: Session = Depends(database_session),
...
@router.post(
    "/{habit_id}/logs", response_model=HabitCompletionResponse, status_code=201
)
